...
import { RootObject, Event } from '../../services/EventsApi';
...
const Events: React.FC<Props> = ({ data }) => {
    ...
    const handleClick = (event: Event) => {
        dispatch(setZoom(15));
        dispatch(setCenter({ lat: Number(event.lat), lng: Number(event.lng) }));
    }
    ...
    return (
        <>
            {data?.events.map((event, i) => {
                ...
                return (
                    <div id={`Event_${event.id}`} className={classes.Single_Event} key={event.id}>
                        ...
                        <div className={classes.Event_Content}>
                            ...
                            <div className={classes.Map_Icon_Container} title="Show on the map">
                                <svg className={classes.Map_icon} onClick={(e) => handleClick(event)}>
                                    <Map_icon />
                                </svg>
                            </div>
                        </div>
